...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      attrJointid(-1), attrWeights(-1), unifBindMatrix(-1), unifOverallTransformation(-1),
      unifColor(-1), unifTime(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJointid = context->glGetAttribLocation(prog, "vs_InfluencerID");
    attrWeights = context->glGetAttribLocation(prog, "vs_Weights");
    unifBindMatrix = context->glGetUniformLocation(prog, "u_BindMatrices");
    unifOverallTransformation = context->glGetUniformLocation(prog, "u_JointTransformation");
...
void ShaderProgram::SetBindMatrix(std::vector<glm::mat4> BindMatrix)
{
    useMe();
    if (unifBindMatrix != -1)
    {
        context->glUniformMatrix4fv(unifBindMatrix,
                                    BindMatrix.size(),
                                    GL_FALSE,
                                    &BindMatrix[0][0][0]);
    }
}
void ShaderProgram::SetOverallTransformation(std::vector<glm::mat4> OverallTransformation)
{
    useMe();
    if (unifOverallTransformation != -1)
    {
        context->glUniformMatrix4fv(unifOverallTransformation,
                                    OverallTransformation.size(),
                                    GL_FALSE,
                                    &OverallTransformation[0][0][0]);
    }
}
